...
// The query dispatcher's pipeline is registered separately from the command pipeline (see [ADR-0039]),UnitOfWorkBehavior runs ONLY on commands,
// so queries skip the transaction wrapper. ValidationBehavior still applies to queries.
