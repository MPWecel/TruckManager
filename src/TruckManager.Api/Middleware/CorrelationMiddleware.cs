...
//   3. Schedule a response-header callback (Response.OnStarting) so the client receives the same id back in X-Correlation-Id — happens just before headers flush, even if the action wrote no body or short-circuited.
//   4. Push CorrelationId + TenantId + UserId into Serilog's LogContext for the rest of the request. Every Log.X / ILogger.LogX call inside the scope gets these properties for free (Enrich.FromLogContext picks them up in Program.cs).
//
// V1 stubs:
...
public sealed class CorrelationMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, IDateTimeProvider clock)
    {
        ...
        // V1 stubs. Pushing the property names (even with null UserId) means logs already carry the schema — when Phase 9 lands, only the source of these values changes; sinks + dashboards do not.
        Guid? userId = null;
