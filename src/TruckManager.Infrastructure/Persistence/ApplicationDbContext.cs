...
// [ADR-0004 / ADR-0010]   The single DbContext for the modular monolith.
// Lives in Infrastructure only; no other layer touches it directly - Application handlers inject IApplicationDbContext (ADR-0040), which is proxied to the same scoped instance.
//
// [ADR-0029]   ConfigureConventions scans the Domain assembly for every closed type implementing IStronglyTypedId<TValue> and registers a generic StronglyTypedIdValueConverter<TId, TValue> for it.
// New aggregate IDs (Phase 5+) pick this up automatically — no per-ID configuration needed.
//
// Per-aggregate mapping lives in IEntityTypeConfiguration<> classes under Persistence/Configurations/, applied via ApplyConfigurationsFromAssembly so adding a new aggregate is a single file drop.
public sealed class ApplicationDbContext(DbContextOptions<ApplicationDbContext> options) : DbContext(options), IApplicationDbContext
{
    ...
    private static void RegisterStronglyTypedIdConverters(ModelConfigurationBuilder configurationBuilder)
    {
        ...
        {
            bool isAbstractOrInterface = idType.IsAbstract || idType.IsInterface;
    ...
    private static bool IsStronglyTypedIdInterface(Type @interface) 
        => @interface.IsGenericType && @interface.GetGenericTypeDefinition().Equals(typeof(IStronglyTypedId<>));
}
